...
from ..schemas.cart import CartResponse, CartItem, CartItemCreat, CartItemUpdate
...
class CartService:
    ...
    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreat) -> Dict[int, int]:
        product = self.product_repository.get_by_id(item.product_id)
        if not product:
            raise HTTPException(
                status_code = status.HTTP_404_NOT_FOUND,
                deteil = f"Product with id {item.product_id} not found"
            )
